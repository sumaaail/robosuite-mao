...
class AssistiveOnePointEnv(TaskInterface):
    ...
    def __init__(self, args, env, set_params):
        ...
        print("delta_pos :", self.tool_delta_pos, "delta_ori :", self.tool_delta_ori)
    ...
    def get_ee_state(self):
        ...
        self.delta_pos = self.target_pos - self.tool_pos + self.tool_delta_pos
        self.mujoco_target_pos = self.mujoco_ee_pose[:3] + self.delta_pos
        self.mujoco_target_euler = self._desired_ori(self.mujoco_ee_pose[3:],
                                                     self.pybullet_ori_euler + self.tool_delta_ori)
    ...
    def send_movement(self, params, n=1):
        """
            send parameters
        """
        self.send_params(params)
        
        reward, context, info = self.run_single_trajectory(n=n)
    ...
    def run_single_trajectory(self, n=None):
        ...
        for i in range(n):
            ...
            while True:
                ...
                action = joint.copy() - joint_last
                
                # env.action_space_human.sample()
                # joint_list.append(joint[0].copy())
                np.putmask(action, action > 3.14, 0.0)
                np.putmask(action, action < -3.14, 0.0)
...
class AssistiveEnv(TaskInterface):
    ...
    def __init__(self, args, env, set_params):
        ...
        if self.args.video_record:
            fps = int(set_params["mujoco_options"]["frame_skip"])
            self.video_writer = imageio.get_writer("{}.mp4".format(self.args.video_path), fps=fps)
            self._env.setup_camera(camera_eye=[0.5, -0.25, 1.5], camera_target=[-0.2, 0, 0.75],
    ...
    def get_ee_state(self):
        ...
        self.mujoco_target_pos = self.mujoco_ee_pose[:3] + self.delta_pos
        self.mujoco_target_euler = self._desired_ori(self.mujoco_ee_pose[3:],
                                                     # np.array([self.pybullet_ori_euler[0] * 0.3, self.pybullet_ori_euler[1], 0.0]) +
                                                     self.pybullet_ori_euler +
                                                     self.tool_delta_ori)
    ...
    def send_params(self, params):
        """
            set relations
        """
        impedance_params = np.clip(params[:self.num_way_points * self.action_dim + self.num_way_points - 1],
                                   self._latent_parameter_low[:self.num_way_points * self.action_dim + self.num_way_points - 1],
                                   self._latent_parameter_high[:self.num_way_points * self.action_dim + self.num_way_points - 1]
                                   )
        print("impedance_params :", impedance_params)
        self.mujoco_model.set_impedance_params(impedance_params)
        # self.delta_pose_params = params[-self.action_dim:]
        self.delta_pose_params = params[-self.action_dim:] * self.delta_pose_scale[-self.action_dim:] \
                                 + self.delta_pose_low[-self.action_dim:]
        print("original_params :", params)
        print("delta_pose_params :", self.delta_pose_params)
