...
class CGPUCB(object):
	def __init__(self, actions, contexts, sample_from_environment, kernel, delta=0.80):
		"""
			...
			sample_from_environment: sampling function which returns observed value.
			kernel: define a kernel using GPy Kernels or you can create one for yourself.
			delta: delta needs to be in interval (0,1) exclusively. Delta is a hyper-parameter that is used to choose
		...
		print("mesh", self.input_mesh[0].shape, self.input_mesh[1].shape)
		print("shape :::", self.input_mesh.shape)
		...
		print("input_space :::", self.input_mesh.reshape(self.input_mesh.shape[0], -1).T.shape)
		self.input_space_size = self.input_space.size
		print("input_size :::", self.input_space_size)
