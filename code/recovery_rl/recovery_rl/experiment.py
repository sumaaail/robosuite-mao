...
from code.pytorch.recovery_rl.recovery_rl.sac import SAC
from code.pytorch.recovery_rl.recovery_rl.replay_memory import ReplayMemory, ConstraintReplayMemory
from code.pytorch.recovery_rl.recovery_rl.MPC import MPC
from code.pytorch.recovery_rl.recovery_rl.VisualMPC import VisualMPC
from code.pytorch.recovery_rl.recovery_rl.model import VisualEncoderAttn, TransitionModel, VisualReconModel
from code.pytorch.recovery_rl.recovery_rl.utils import linear_schedule, recovery_config_setup
from code.pytorch.recovery_rl.env.make_utils import register_env, make_env
...
class Experiment:
    def __init__(self, exp_cfg):
        ...
        self.constraint_demo_data, self.task_demo_data, self.obs_seqs, self.ac_seqs, self.constraint_seqs = \
        ...
        if not self.exp_cfg.vismpc_recovery:
            ...
            for i in range(self.exp_cfg.critic_safe_pretraining_steps):
                ...
                self.agent.safety_critic.update_parameters(
                    ...
                    batch_size=min(self.exp_cfg.batch_size, len(self.constraint_demo_data))
                )
