...
class QRiskWrapper:
    def __init__(self, obs_space, ac_space, hidden_size, logdir,
        ...
        if not self.images:
            ...
            self.policy = StochasticPolicyCNN(obs_space,
                                              ac_space.shape[0],
    ...
    def update_parameters(self,
        ...
        if self.pos_fraction:
            batch_size = min(batch_size, int((1 - self.pos_fraction) * len(memory)))
        else:
        ...
        # print("next_state_action :::", next_state_batch.shape)
        # print("next_state_log_pi :::", state_batch.shape)
        if self.encoding:
        ...
        with torch.no_grad():
            next_state_action, next_state_log_pi, _ = \
                policy.sample(next_state_batch)
        ...
        if self.updates % self.target_update_interval == 0:
            soft_update(self.safety_critic_target,
                        self.safety_critic,
                        self.tau
                        )
        self.updates += 1
