...
class ReplayBuffer(object):
    ...
    def save(self, filename):
        ...
        np.save(filename + ".npy", self.storage)
    ...
    def load_buffer(self, data):
        ...
        for i in range(data['state'].shape[0] - 1):
            self.add(data['state'][i], data['action'][i], data['next_state'][i],
                     data['reward'][i], 1 - data['not_done'][i])
        print("Dataset size:" + str(self.size))