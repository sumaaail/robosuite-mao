...
import copy
import pickle
from code.pytorch.PLAS.logger import logger
from code.pytorch.PLAS.logger import create_stats_ordered_dict
...
class VAEModule(object):
    ...
    def train_step(self, dataset, batch_size=100):
        dataset_size = len(dataset['state'])
        ind = np.random.randint(0, dataset_size, size=batch_size)
        state = dataset['state'][ind]
        action = dataset['action'][ind]
